...
public enum ResistingModule implements ModifierModule, ProjectileHitModifierHook {
    INSTANCE;
    private static final List<ModuleHook<?>> DEFAULT_HOOKS = HookProvider.defaultHooks(ModifierHooks.PROJECTILE_HIT);
    public static final RecordLoadable<ResistingModule> LOADER = new SingletonLoader<>(INSTANCE);

    public RecordLoadable<ResistingModule> getLoader() {
        return LOADER;
    }
    ...
    public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target, boolean notBlocked) {
        ...
        if (target.getType().is(ModTags.EntityTypes.resisting) && projectile instanceof AbstractArrow arrow && attacker != null) {
                ...
                if (arrow.isCritArrow()) {
                    damage = (int) Math.min(Modifier.RANDOM.nextInt(damage / 2 + 2) + (long) damage, Integer.MAX_VALUE);
                }
                ...
                if (target.hurt(damageSource, (float) damage)) {
                 ...
                 if (!level.isClientSide && arrow.shotFromCrossbow() && owner instanceof ServerPlayer player) {
                    ...
                    arrow.playSound(arrow.soundEvent, 1.0F, 1.2F / (Modifier.RANDOM.nextFloat() * 0.2F + 0.9F));
                    if (arrow.getPierceLevel() <= 0) {
                        arrow.discard();
                    }
                    else {
