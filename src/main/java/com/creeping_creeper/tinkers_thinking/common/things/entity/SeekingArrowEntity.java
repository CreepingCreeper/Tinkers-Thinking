...
public class SeekingArrowEntity extends AbstractArrow {
    ...
    public SeekingArrowEntity(EntityType<SeekingArrowEntity> entityType, Level level) {
    ...
    public SeekingArrowEntity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(ModEntities.Seeking_Arrow.get(), level);
            ...
            if (id == -1) {
                ...
                if (arcTowards != null) {
                    ...
                    if(arcVec.length() > arcTowards.getBbWidth()){
                        this.setDeltaMovement(this.getDeltaMovement().scale(0.31).add(arcVec.normalize().scale(0.7)));
                    }
                }
            }
        }
    ...
    public @NotNull ItemStack getPickupItem() {
        return new ItemStack(ModToolItems.seeking_arrow.get());
    }
