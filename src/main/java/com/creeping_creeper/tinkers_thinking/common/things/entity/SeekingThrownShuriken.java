...
public class SeekingThrownShuriken extends ThrownShuriken {
    //The code in AlexModGuy's AlexsCaves mod was used and modified.
    private static final EntityDataAccessor<Integer> ARC_TOWARDS_ENTITY_ID = SynchedEntityData.defineId(SeekingThrownShuriken.class, EntityDataSerializers.INT);
    public SeekingThrownShuriken(EntityType<? extends ThrownShuriken> type, Level level) {
        super(type, level);
    }
    public SeekingThrownShuriken(Level level, double x, double y, double z) {
        super(level,x, y, z);
    }
